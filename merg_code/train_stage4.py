...
def parser_args():
    ...
    parser.add_argument('--cs_ckpt', type=str, default=None)
...
def main(**args):
    ...
    # ===== load cs / sd checkpoint (optional) =====
    if args.get('cs_ckpt') is not None:
        cs.load_state_dict(torch.load(args['cs_ckpt'], map_location=device))
        if args['local_rank'] == 0:
            print(f"[INFO] Loaded CS ckpt: {args['cs_ckpt']}")

    if args.get('sd_ckpt') is not None:
        sd.load_state_dict(torch.load(args['sd_ckpt'], map_location=device))
        if args['local_rank'] == 0:
            print(f"[INFO] Loaded SD ckpt: {args['sd_ckpt']}")
    ...
    if is_main_process:
        ...
        print(f'Save final ckpt at {ckpt_dir}')
